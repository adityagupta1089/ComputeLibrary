all: release debug
debug:
	scons -j8 -Q arch=arm64-v8a build=native neon=1 opencl=1 build_dir=debug debug=1
release:
	scons -j8 -Q arch=arm64-v8a build=native neon=1 opencl=1 build_dir=release debug=0
al2:
