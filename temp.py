...
targets = {
    "cpu big": lambda g: bg + [pp + g] + cp,
    "cpu small": lambda g: sm + [pp + g] + cp,
...
temp_func = lambda t, q0, c, v, r: q0 + c * v * (1 - np.exp(- t / (r * c)))
...
def plot_temp(graph, target, x, y, popt, i):
    ...
    plt.plot(x, temp_func(x, *popt), 
        'r-', 
        label='fit: Q0=%5.3f, C=%5.3f, V=%5.3f, R=%5.3f' % tuple(popt))
...
def getRC(graph, target, x, y, i):
    popt, pcov = curve_fit(temp_func, x, y, bounds=(0, np.inf))
    print('> Q0=%5.3f, C=%5.3f, V=%5.3f, R=%5.3f' % tuple(popt))
    plot_temp(graph, target, x, y, popt, i)
       
def findRC(graph, target, cmd, i):
    x = []
    y = []
    t = 0
    time.sleep(T)
    ...
    x = np.array(x)
    y = np.array(y)
    return getRC(graph, target, x, y, i)
...
if __name__ == "__main__":
    ...
    for graph in graphs[:1]:
        ...
        for target in targets:
            ...
            for i in range(1):
                print("Iteration", i)
                RCs[(graph, target)] = findRC(graph, target, targets[target](graph), i)
            break
